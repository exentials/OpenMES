...
public class ProductionDeclarationIntegrationTests
	: IClassFixture<WebApplicationFactory<Program>>
{
	...
	public ProductionDeclarationIntegrationTests(WebApplicationFactory<Program> factory)
	{
		_factory = factory.WithWebHostBuilder(builder =>
		{
			builder.ConfigureServices(services =>
			{
				...
				services.AddDbContextPool<OpenMESIdentityDbContext>(options =>
					options.UseInMemoryDatabase("IntegrationIdentityDb"));
	...
	public async Task GetAll_WithoutAuth_Returns401()
	{
		...
		var response = await client.GetAsync("/productiondeclaration");
	...
	public async Task GetAll_WithAuth_Returns200()
	{
		...
		var response = await client.GetAsync("/productiondeclaration");
		var authDetails = string.Join("; ", response.Headers.WwwAuthenticate.Select(h => h.ToString()));

		Assert.True(response.IsSuccessStatusCode, $"Status: {response.StatusCode}; Auth: {authDetails}");
	}

	[Fact]
	public async Task GetPendingExport_WithAuth_Returns200()
	{
		using var client = CreateAuthenticatedClient();

		var response = await client.GetAsync("/productiondeclaration/pending-export");
		var authDetails = string.Join("; ", response.Headers.WwwAuthenticate.Select(h => h.ToString()));
	...
	private static void RemoveDbContextRegistrations<TContext>(IServiceCollection services)
		...
		=> IntegrationTestHelpers.RemoveDbContextRegistrations<TContext>(services);
}
